...
public class UpsertOracleSecurityGroupAtomicOperation implements AtomicOperation<Void> {
  ...
  private void create() {
    ...
    CreateSecurityListResponse response = description.getCredentials().getNetworkClient().createSecurityList(request);
    String securityListId = response.getSecurityList().getId();

    getTask().updateStatus(BASE_PHASE, "Finished creating security group " + getDescription().getSecurityGroupName());

    if (description.getSubnetIds() != null && !description.getSubnetIds().isEmpty()) {
      for (String subnetId : description.getSubnetIds()) {
        addSecurityListToSubnet(securityListId, subnetId);
      }
    }
  }

  private void addSecurityListToSubnet(String securityListId, String subnetId) {
    getTask().updateStatus(BASE_PHASE, "Adding security group " + getDescription().getSecurityGroupName()
            + "to subnet " + subnetId);

    GetSubnetResponse response =
            description.getCredentials().getNetworkClient().getSubnet(GetSubnetRequest.builder().subnetId(subnetId).build());
    List<String> securityListIds = response.getSubnet().getSecurityListIds();
    if (securityListIds == null) {
      securityListIds = new ArrayList<String>();
    }
    securityListIds.add(securityListId);

    UpdateSubnetDetails details = UpdateSubnetDetails.builder().securityListIds(securityListIds).build();
    description.getCredentials().getNetworkClient().updateSubnet(
            UpdateSubnetRequest.builder()
                    .subnetId(subnetId)
                    .updateSubnetDetails(details)
                    .build());

    getTask().updateStatus(BASE_PHASE, "Finished adding security group " + getDescription().getSecurityGroupName()
            + "to subnet " + subnetId);
  }
  ...
  private void update() {
    ...
    UpdateSecurityListDetails details = UpdateSecurityListDetails.builder()
            ...
            .egressSecurityRules(buildEgressRules())
            .build();
    UpdateSecurityListRequest request = UpdateSecurityListRequest.builder()
            .securityListId(description.getSecurityGroupId())
            .updateSecurityListDetails(details).build();
    description.getCredentials().getNetworkClient().updateSecurityList(request);

    // TODO what if the subnetIds changed?
